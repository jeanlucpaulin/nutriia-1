...
public class MicronutrientsOfMyDay extends AppFragment implements OnValidateDay, APIResponseRDA {
    ...
    private void update() {
        ...
        if(DISPLAY_ALL_ITEMS) nutrientsList.addAll(micronutrients);
        else {
            for(int i = 0; i < Settings.getMaxDisplayedItems() && i < micronutrients.size(); i++) {
                nutrientsList.add(micronutrients.get(i));
            }
        }
